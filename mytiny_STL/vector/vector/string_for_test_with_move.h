...
namespace myString {
    ...
    public:
        ...
        string(const string& s) :_str(nullptr) {
            puts("string(const string& s)");
            string tmp(s._str);
            swap(forward<string>(tmp));
        ...
        string(string&& s) {
            puts("string(string&& s)");
            swap(forward<string>(s));

        }
        //两个已经存在的对象赋值（深拷贝）
        string& operator=(const string& s) {   //调用拷贝构造 string tmp = s;
            puts("string& operator=(string tmp)");
            string tmp(s);
            swap(forward<string>(tmp));
            return *this;
        }
        string& operator=(string&& s) {
            puts("string& operator=(string&& s)");
            swap(forward<string>(s));
            return *this;
        }
